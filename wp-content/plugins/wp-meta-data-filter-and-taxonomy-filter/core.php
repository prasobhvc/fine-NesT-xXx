...
//if not multi val range-slider
    ...
    {
        global $wpdb;
        $sql = "TRUNCATE TABLE " . self::$mdf_query_cache_table;
        $wpdb->query($sql);
        //wp_die('done');
    }
...
class WP_QueryMDFCache extends WP_Query
{
    ...
    private function set_cache_value()
    {
        ...
        $wpdb->query($wpdb->prepare("INSERT INTO {$this->cache_table} (post_id, meta_key, meta_value) VALUES ( %d, %s, %s)", 0, $this->key_string, $ids));
    }
    ...
    private function get_cache_value()
    {
        global $wpdb;
        $result = -1;
        $sql = $wpdb->prepare("SELECT meta_key,meta_value FROM {$this->cache_table} WHERE meta_key=%s", $this->key_string);
