...
final class MDTF_HELPER
{
    ...
    public static function mdf_post_title_or_content_filter($where = '')
    {
        ...
        if (isset($data[2]) AND ! empty($data[2]))
        {
            ...
            if ($data[0] == 'LIKE')
            {
                $where .= "AND (post_content LIKE '%{$search}%' OR post_title LIKE '%{$search}%') OR post_excerpt LIKE '%{$search}%')";
            } else
            {
                $where .= "AND (post_content='{$search}' OR post_title='{$search}' OR post_excerpt='{$search}')";
            }
        }
