...
/*
  ...
  Requires at least: WP 3.8.0
  Tested up to: WP 4.4.2
  Author: realmag777
  Author URI: http://pluginus.net/
  Version: 1.2.1
  Author URI: http://www.pluginus.net/
  Tags: filter, search, woocommerce, products filter, filter of products, posts filter, taxonomies filter, taxonomies widget
    ...
    public static function cron_schedules($schedules)
    {
        ...
        for ($i = 2; $i <= 7; $i++)
        {
            $schedules['days' . $i] = array(
                ...
                'display' => sprintf(__("each %s days", 'meta-data-filter'), $i)
            );
        }
...
//for recount
    ...
    public static function admin_menu()
    {
        ...
        if (class_exists('MDTF_UTILITIES'))
        {
            add_submenu_page('edit.php?post_type=' . self::$slug, __("MDTF Utilities", 'meta-data-filter'), __("MDTF Utilities", 'meta-data-filter'), 'manage_options', 'mdf_utilities', array(__CLASS__, 'draw_utilities_page'));
        }
    }
...
//watch for template file existing
    ...
    public static function data_meta_box_style()
    {
        ...
        <h4><?php _e("Show as multiple checkbox select", 'meta-data-filter') ?>:</h4>
        <?php
        $choice = array(
        ...
        <p class="description">
            <?php _e("Example", 'meta-data-filter') ?>: <a href="http://codepen.io/elmahdim/pen/hlmri" target="_blank">http://codepen.io/elmahdim/pen/hlmri</a><br />
            <?php _e("Uses ONLY for current section with checkboxes. Not for taxonomies ...", 'meta-data-filter') ?><br />
        </p>
        <?php
    }
...
//ajax - for option of any taxonomy in the widget
    ...
    public static function get_terms($taxonomy, $hide_empty = false, $get_childs = true, $selected = 0, $category_parent = 0)
    {
        ...
        $orderby = apply_filters('mdf_terms_orderby', 'name');
        $order = apply_filters('mdf_terms_order', 'ASC');
...
//print_r($widget_options);
    ...
    private static function draw_term_multi_select($terms, $mdf_parent_id = 0, $selected_ids = array(), $hide, $tax_name = 0, $is_ajax = true, $hide_string_ids = '', $widget_options = array())
    {
        ?>
        <h3>Premium only! Get your copy of the plugin from CodeCanyon!</h3>
        <a href="http://codecanyon.net/item/wordpress-meta-data-taxonomies-filter/7002700?ref=realmag777" alt="Get the plugin" target="_blank"><img src="<?php echo self::get_application_uri() ?>images/mdtf_banner.jpg" /></a><br />
        <br />
        <?php
    }
