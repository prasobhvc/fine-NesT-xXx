...
<div style="display:none;" class="mdf_admin_flter_item_box">
    ...
    <div class="data_group_item_html">
        ...
        <div class="data_group_item_template_slider data_group_input_type" style="display: <?php echo(isset($itemdata) ? ($itemdata['type'] == 'slider' ? "block" : "none") : "none") ?>">
            ...
            <i><?php _e("Enabling this mode allows you to set 2 values in post for range-slider - RANGE 'From' and 'To'. Not possible to reflect this. On the front range-slider is single! Doesn work with WooCommerce functionality.", 'meta-data-filter') ?></i>
        ...
        <div class="data_group_item_template_select data_group_input_type" style="display: <?php echo(isset($itemdata) ? ($itemdata['type'] == 'select' ? "block" : "none") : "none") ?>;">
                ...
                <?php if (isset($itemdata)): ?>
                    ...
                    <?php if (!empty($itemdata['select'])): ?>
                        <?php foreach ($itemdata['select'] as $k => $value) : ?>
                            <li>
                                <input type="text" name="html_item[<?php echo $uniqid ?>][select][]" value="<?php echo $value ?>">&nbsp;<input type="text" class="data_item_select_option_key" placeholder="<?php _e("key for the current option - must be unique", 'meta-data-filter') ?>" name="html_item[<?php echo $uniqid ?>][select_key][]" value="<?php echo((isset($itemdata['select_key'][$k]) AND ! empty($itemdata['select_key'][$k])) ? /* sanitize_title */trim($itemdata['select_key'][$k]) : /* sanitize_title */trim($value)) ?>" style="width: 25%;">&nbsp;<a class="delete_option_from_data_item_select remove-button" href="#"></a>
                                &nbsp;<img width="15" style="vertical-align: middle;" src="<?php echo MetaDataFilter::get_application_uri() ?>images/drag_and_drope.png" title="<?php _e("drag and drope", 'meta-data-filter') ?>" alt="<?php _e("drag and drope", 'meta-data-filter') ?>" />
                                <br />
                            </li>
                        <?php endforeach; ?>
                    <?php endif; ?>
            ...
            <br />
            <h4><?php _e("Sort options by alphabetical order", 'meta-data-filter') ?></h4>
            ...
            <select name="html_item[<?php echo $uniqid ?>][select_sort_value_by_alphabetical]">
                <option value="0" <?php if ($select_sort_value == 0): ?>selected=""<?php endif; ?>><?php _e("No", 'meta-data-filter') ?></option>
                <option value="1" <?php if ($select_sort_value == 1): ?>selected=""<?php endif; ?>><?php _e("Yes", 'meta-data-filter') ?></option>
            </select><br />
            <i><?php _e("Sort options by alphabetical order on the front", 'meta-data-filter') ?></i>
