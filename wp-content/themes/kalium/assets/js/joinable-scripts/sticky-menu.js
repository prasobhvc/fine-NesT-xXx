;(function($, window, undefined)
{
	...
	$(document).ready(function()
	{
		...
		var $header = $("header.main-header"),
			...
			immediateStickyActivate = $header.hasClass( 'sticky-immediate-activate' );
		...
		if($header.hasClass('is-sticky'))
		{
			...
			if(headerOptions && headerOptions.stickyUseCustomLogo)
			{
				...
				{
					var headerLogoExitScrollEvent = 'exitViewport',
						headerLogoEnterScrollEvent = 'enterViewport';
					
					if( immediateStickyActivate ) {
						headerLogoExitScrollEvent = 'partiallyExitViewport';
						headerLogoEnterScrollEvent = 'fullyEnterViewport';
					}
					
					headerWatcher[headerLogoExitScrollEvent](function()
					{
					...
					headerWatcher[headerLogoEnterScrollEvent](function()
					{
						revertHeaderLogo();
					});
				};
			...
			// Create Watcher
			var headerWatcher = scrollMonitor.create($header[0], {top: (immediateStickyActivate ? 0 : stickyMenuHeight/2)});
			...
			// Bug fix
			if( immediateStickyActivate ) {
				headerWatcher.unlock();
				headerWatcher.recalculateLocation();
				headerWatcher.lock();
			}
