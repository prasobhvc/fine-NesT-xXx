...
function laborator_wp_enqueue_scripts() {
	...
	if ( get_data( 'use_custom_skin' ) ) {
		...
		if ( '' != get_option( 'permalink_structure' ) && true != get_data( 'theme_skin_include_alternate' ) ) {
			wp_enqueue_style( 'custom-skin', site_url( 'skin.css' ), null, null );
		} else {
...
add_action( 'init', 'lab_custom_skin_rewrite' );
...
add_action( 'template_redirect', 'lab_custom_skin_template_redirect', 1 );
...
function laborator_admin_enqueue_scripts() {
	...
	?>
	<script type="text/javascript">var kalium_assets_dir = "<?php echo THEMEASSETS; ?>";</script>
	<?php
}
...
function laborator_get_paged_portfolio_items() {
	...
	$resp['content'] = $content;
	$resp['qq'] = $query_args;
	$resp['url'] = admin_url( 'admin-ajax.php' ) . '?' . http_build_query( $_REQUEST );
...
function check_for_custom_logo_in_page() {
	...
	$custom_menu_skin      = get_field( 'custom_menu_skin' );
	$sticky_menu_on_page   = get_field( 'sticky_menu_on_page' );
