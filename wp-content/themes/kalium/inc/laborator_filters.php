...
function shortcode_lab_social_networks($atts = array(), $content = '')
{
	...
	$social_order_list = array(
		...
		"vm"  => array("title" => __("Vimeo", 'kalium'), 			"icon" => "fa fa-vimeo"),
		...
		"sc"  => array("title" => __("SoundCloud", 'kalium'), 		"icon" => "fa fa-soundcloud"),
		"hz"  => array("title" => __("Houzz", 'kalium'), 			"icon" => "fa fa-houzz"),
		"px"  => array("title" => __("500px", 'kalium'), 			"icon" => "fa fa-500px"),
	...
	$html = '<ul class="social-networks list-unstyled list-inline social'.(isset($atts['class']) ? esc_attr(' ' . $atts['class']) : '').'">';
	...
	foreach($social_order['visible'] as $key => $title)
	{
		...
		$html .= '<li>';
			$html .= '<a href="'.$href.'" target="_blank" class="color '.(preg_match("/^[0-9]+/", $class) ? 'social-' : '').$class.'" title="'.$title.'">';//" target="_blank">';
...
function lab_wp_title_parts( $title, $sep, $seplocation ) {
	...
	if( $seplocation == 'right' ) {
		$title = str_replace( $sep . end( $title_sep ), " &ndash; " . end( $title_sep ), $title );
	} else {
		$title = str_replace( reset( $title_sep ) . $sep, reset( $title_sep ) . " &ndash; ", $title );
	}
...
// Current Portfolio Highlight (Bug fix)
function portfolio_current_nav_class( $classes, $item ) {
	...
	if( $path_info['filename'] == 'portfolio' ) {
		$classes[] = 'current-menu-item current_page_item';
	}
	
	$classes[] = 'xxx-' . $path_info['filename'];
...
if( ! empty( $_SERVER['REQUEST_URI'] ) ) {
	...
	if( ! empty( $req_path_info['filename'] ) && $req_path_info['filename'] == 'portfolio' ) {
...
if ( trim ( get_data( 'portfolio_prefix_url_slug' ) ) ) {
	
	add_filter( 'portfolioposttype_args', 'portfolio_rewrite_custom_url_slug' );
	
	function portfolio_rewrite_custom_url_slug( $args ) {
