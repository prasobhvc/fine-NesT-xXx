...
<?php
wp_enqueue_style('mdf_table2', get_template_directory_uri() . '/mdf_templates/woocommerce/table2/css/styles.css');
global $mdf_loop;
MDTF_SORT_PANEL::mdtf_catalog_ordering();
?>
<table id="racetimes">
    <tr id="firstrow">
        <td><?php _e('Title', 'meta-data-filter') ?></td>
        <td><?php _e('Price', 'meta-data-filter') ?></td>
        <td><?php _e('Description', 'meta-data-filter') ?></td>
        <td><?php _e('View', 'meta-data-filter') ?></td>
    </tr>
    <?php
    while ($mdf_loop->have_posts()) : $mdf_loop->the_post();
        ...
        <tr>
            <td style="width: 30%;">
                <a href="<?php the_permalink() ?>" target="_blank" class="thumbnail alignleft"><img src="<?php echo MDTF_HELPER::get_post_featured_image($post->ID, '200x180') ?>" alt="<?php the_title() ?>" /></a><br />
            ...
            <td style="width: 5%;"><?php
                ...
                ?></td>
            <td style="width: 60%; text-align: left;"><?php echo $post->post_excerpt; ?><br />
                <br />
                <?php
