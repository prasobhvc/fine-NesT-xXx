...
if ( preg_match( "/^[a-z_-]+$/i", get_data( 'portfolio_thumbnail_size_2' ) ) ) {
	$portfolio_image_size = get_data( 'portfolio_thumbnail_size_2' );
}
...
if ( $box_size ) {
	...
	$box_h3 = floor( $box_h1 * 3 );
	$box_h4 = floor( $box_h1 * 4 );
	$box_h5 = floor( $box_h1 * 5 );
	$box_h6 = floor( $box_h1 * 6 );
...
$hover_state_class[] = 'hover-' . $portfolio_type_2_hover_effect;
...
?>
<div <?php post_class( $item_class ); ?><?php if ( $portfolio_terms_slugs ): ?> data-terms="<?php echo implode( ' ', $portfolio_terms_slugs ); ?>"<?php endif; ?>>
    	...
    	<div class="product-box <?php echo esc_attr( $show_effect ); ?>"<?php if ( $reveal_delay ): ?> data-wow-delay="<?php echo esc_attr( $reveal_delay ); ?>s"<?php endif; ?>>
	    	<div class="thumb">
				...
				<a href="<?php echo esc_url( $portfolio_item_href ); ?>" <?php if ( $portfolio_item_new_window ): ?> target="_blank"<?php endif; ?>>
					<?php laborator_show_image_placeholder( $post_thumbnail_id, $portfolio_image_size, 'do-lazy-load-on-shown' ); ?>
				</a>
    		</div>
    	</div>
