		...
		<div id="payment">
					...
					if ( $available_gateways = WC()->payment_gateways->get_available_payment_gateways() ) {
						...
						foreach ( $available_gateways as $gateway ) {
							...
							<li class="payment_method_<?php echo $gateway->id; ?>">
								...
								<input id="payment_method_<?php echo $gateway->id; ?>" type="radio" class="input-radio replaced-radio-buttons" name="payment_method" value="<?php echo esc_attr( $gateway->id ); ?>" <?php checked( $gateway->chosen, true ); ?> data-order_button_text="<?php echo esc_attr( $gateway->order_button_text ); ?>" />
								<label for="payment_method_<?php echo $gateway->id; ?>"><?php echo $gateway->get_title(); ?> <?php echo $gateway->get_icon(); ?></label>
								<?php
									if ( $gateway->has_fields() || $gateway->get_description() ) {
										echo '<div class="payment_box payment_method_' . $gateway->id . '" style="display:none;">';
										echo $gateway->get_icon(); // Added this / by Arlind Nushi
			...
			<div class="form-row">
				...
				<?php
					...
					echo apply_filters( 'woocommerce_pay_order_button_html', '<input type="submit" class="button alt btn btn-primary" id="place_order" value="' . esc_attr( $pay_order_button_text ) . '" data-value="' . esc_attr( $pay_order_button_text ) . '" />' );
