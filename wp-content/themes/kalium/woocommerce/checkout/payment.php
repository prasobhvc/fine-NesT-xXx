	...
	<div class="form-row place-order">
		...
		<?php if ( wc_get_page_id( 'terms' ) > 0 && apply_filters( 'woocommerce_checkout_show_terms', true ) ) : ?>
			<p class="form-row terms">
				<?php # start: modified by Arlind Nushi ?>
				<input type="checkbox" class="input-checkbox replaced-checkboxes" name="terms" <?php checked( apply_filters( 'woocommerce_terms_is_checked_default', isset( $_POST['terms'] ) ), true ); ?> id="terms" />
		...
		<?php # start: modified by Arlind Nushi ?>
		<?php echo apply_filters( 'woocommerce_order_button_html', '<input type="submit" class="button alt btn btn-primary btn-lg btn-block" name="woocommerce_checkout_place_order" id="place_order" value="' . esc_attr( $order_button_text ) . '" data-value="' . esc_attr( $order_button_text ) . '" />' ); ?>
		<?php # end: modified by Arlind Nushi ?>
